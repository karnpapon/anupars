...
impl Rect {
  ...
  pub fn offset_subtract_y(&mut self) {
    self.top_left = self.top_left.checked_sub((0, 1)).unwrap();
    self.bottom_right = self.bottom_right.checked_sub((0, 1)).unwrap();
  }

  /// Adds the given offset to this rectangle.
  pub fn offset_subtract_x(&mut self) {
    self.top_left = self.top_left.checked_sub((1, 0)).unwrap();
    self.bottom_right = self.bottom_right.checked_sub((1, 0)).unwrap();
  }
