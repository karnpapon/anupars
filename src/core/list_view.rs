use cursive::builder::{Config, Context, Error, Resolvable};
use cursive::Rect;
use cursive::{
  direction,
...
/// Represents a child from a [`CustomListView`].
pub enum ListChild {
...
// Implement `Default` around `CustomListView::new`
// new_default!(CustomListView);

impl CustomListView {
  /// Creates a new, empty `CustomListView`.
  pub fn new() -> Self {
    CustomListView {
      children: Vec::new(),
...
impl View for CustomListView {
  ...
  fn on_event(&mut self, event: Event) -> EventResult {
    ...
    res.and(match event {
      Event::Key(Key::Up) if self.focus > 0 => self.move_focus(1, direction::Direction::down()),
...
#[cursive::blueprint(CustomListView::new())]
struct Blueprint {
  children: Vec<ListChild>,
