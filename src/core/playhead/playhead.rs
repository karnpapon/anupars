...
use super::midi::MidiHandlerConfig;
use super::midi::MidiTriggerHandler;
use super::movement::Movement;
use super::position::PositionCalculator;
use super::queue::QueueManager;
use super::tilt::TiltMode;
use super::types::SweepRowMode;
use super::types::{Direction, GridState, Message, ModeFlags, MusicState, UIUpdate};

pub struct Playhead {
  /// position of the top-left corner of the playhead area in the grid
  pub(super) pos: Arc<Mutex<Vec2>>,
  /// area of the playhead, defined by the top-left corner (pos) and its width and height
  pub(super) area: Arc<Mutex<Rect>>,
  pub(super) drag_start_x: AtomicUsize,
  pub(super) drag_start_y: AtomicUsize,
  /// current running position of the playhead within, relative to the playhead area (0,0)
  pub(super) actived_pos: Arc<Mutex<Vec2>>,
  /// position captured when freeze mode was enabled; MIDI retriggers here while frozen
  pub(super) frozen_active_pos: Arc<Mutex<Vec2>>,
  /// aimed area for Ctrl+hjkl aiming feature
  pub(super) aimed_area: Arc<Mutex<Option<Rect>>>,

  // Consolidated state
  pub(super) grid: GridState,
  pub(super) music: MusicState,
  pub modes: ModeFlags,

  // Pattern matching state
  pub(super) regex_indexes: Arc<Mutex<BTreeSet<usize>>>,
  pub(super) text_matcher: Arc<Mutex<Option<HashMap<usize, Match>>>>,

  // Subsystems
  pub(super) queue_manager: Arc<QueueManager>,
  pub(super) position_calc: Arc<PositionCalculator>,
  pub(super) midi_handler: Arc<MidiTriggerHandler>,

  // Accumulation mode state
  pub(super) accumulation_counter: Arc<Mutex<usize>>,
  pub(super) movement: Arc<Mutex<Movement>>,
  pub(super) tilt_mode: Arc<Mutex<TiltMode>>,
  pub(super) sweep_row_mode: Arc<Mutex<SweepRowMode>>,

  pub(super) easing_mode: Arc<Mutex<EasingMode>>,
  /// Absolute tick at which the next step should fire (accumulator-based timing).
  pub(super) next_step_tick: Arc<AtomicUsize>,
  /// Last tick seen - used to detect clock regression (reset) and clear the accumulator.
  pub(super) prev_tick: Arc<AtomicUsize>,

  // Threading/sync state
  pub ui_tx: Sender<UIUpdate>,
  pub(super) step_index: Arc<Mutex<usize>>,
  pub(super) ratchet_generation: Arc<AtomicUsize>,
  /// Remaining fast-division steps within the current regex match span.
  pub(super) match_span_remaining: Arc<AtomicUsize>,
  ...
  pub synth_cc: crate::state::SynthCcShared,
...
impl Playhead {
  pub fn new(
    ...
    synth_cc: crate::state::SynthCcShared,
  ) -> Self {
  ...
  pub(super) fn execute_front_op_in_queue(&self) {
    use super::queue::{EventOperator, QueueItem};
      ...
      while let Ok(control_message) = rx.try_recv() {
        match control_message {
          ...
          Message::ToggleForwardMode() => {
            self.switch_movement(super::movement::Movement::Forward);
          }
          Message::ToggleReverseMode() => {
            self.switch_movement(super::movement::Movement::Reverse);
          }
          Message::ToggleArpeggiatorMode() => {
            self.toggle_arpeggiator_mode();
          }
          Message::ToggleRandomMode() => {
            self.switch_movement(super::movement::Movement::Random);
          }
          Message::TogglePendulumMode() => {
            self.switch_movement(super::movement::Movement::Pendulum);
          }
...
mod tests {
  ...
  fn chn_str_2x2_splits_maps_quadrants_correctly() {
    ...
    assert_eq!(ph.compute_chn_str(Vec2::new(0, 0)), "1/4");
    assert_eq!(ph.compute_chn_str(Vec2::new(4, 0)), "2/4");
    assert_eq!(ph.compute_chn_str(Vec2::new(0, 4)), "3/4");
    assert_eq!(ph.compute_chn_str(Vec2::new(4, 4)), "4/4");
  }
